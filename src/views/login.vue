<template>
<div class="s-col00">
	<div class="m-lg">
		<div class="g-lg">
			<div class="m-lg-nav">
<<<<<<< HEAD
				<img src="../../static/images/lg_logo.png" class="u-logo">
=======
				<img src="img/lg_logo.png" class="u-logo">
>>>>>>> 1c69586bef580f1be2d18659b8b32b1873a7d1d9
				<ul class="r">
					<li class="on">产品介绍</li>
					<li>关于我们</li>
					<li>成功案例</li>
				</ul>
			</div>
<<<<<<< HEAD
			<img src="../../static/images/lg_against.png" class="u-dzt">
			<img src="../../static/images/lg_jb.png" class="u-jb">
=======
			<img src="img/lg_against.png" class="u-dzt">
			<img src="img/lg_jb.png" class="u-jb">
>>>>>>> 1c69586bef580f1be2d18659b8b32b1873a7d1d9
			<div class="m-lg-box">
				<p class="u-p-lg">用户登录</p>
				<form class="mt56">
					<div class="g-ipt-lg"><input type="text" name="telephone" placeholder="用户名/手机号码" class="u-ipt-lg"></div>
					<div class="g-ipt-lg"><input type="text" name="password" placeholder="密码" class="u-ipt-lg"></div>
					<div class="g-forget">
<<<<<<< HEAD
						<label class="l re" for="fg">
							<input type="checkbox" name="" id="fg">记住我
=======
						<label class="l re">
							<input type="checkbox" name="">记住我
>>>>>>> 1c69586bef580f1be2d18659b8b32b1873a7d1d9
							<i class="u-mind-cb"></i>
						</label>
						<span class="r">忘记密码？</span>
					</div>
				</form>
				<button type="button" class="u-lg-btn">登&nbsp&nbsp录</button>
			</div>
			<ul class="g-ts clearfix">
				<li>
<<<<<<< HEAD
					<img src="../../static/images/lg_ts1.png">
=======
					<img src="img/lg_ts1.png">
>>>>>>> 1c69586bef580f1be2d18659b8b32b1873a7d1d9
					<p class="s-ts-tit">多元化赛制模板</p>
					<p class="mt10">高效便捷</p>
					<p class="mt2">一键生成赛事</p>
				</li>
				<li>
<<<<<<< HEAD
					<img src="../../static/images/lg_ts4.png">
=======
					<img src="img/lg_ts4.png">
>>>>>>> 1c69586bef580f1be2d18659b8b32b1873a7d1d9
					<p class="s-ts-tit">赛事进程管理</p>
					<p class="mt10">透明比赛进程</p>
					<p class="mt2">记录每个场次每局比分</p>
				</li>
				<li>
<<<<<<< HEAD
					<img src="../../static/images/lg_ts3.png">
=======
					<img src="img/lg_ts3.png">
>>>>>>> 1c69586bef580f1be2d18659b8b32b1873a7d1d9
					<p class="s-ts-tit">报名选手信息管理</p>
					<p class="mt10">开放报名入口</p>
					<p class="mt2">收集报名信息不再苦恼</p>
				</li>
				<li>
<<<<<<< HEAD
					<img src="../../static/images/lg_ts2.png">
=======
					<img src="img/lg_ts2.png">
>>>>>>> 1c69586bef580f1be2d18659b8b32b1873a7d1d9
					<p class="s-ts-tit">个性化个人主页</p>
					<p class="mt10">规范赛事</p>
					<p class="mt2">建立统一赛事品牌</p>
				</li>
			</ul>
		</div>
		<div class="footer">
			<p>Copyright © 2016 浙江网竞网络科技有限公司未经许可不得复制、转载或摘编，违者必究！</p>
			<p class="mt6">浙ICP备13005497号-5</p>
<<<<<<< HEAD
			<img src="../../static/images/footer.png" class="mt32">
=======
			<img src="img/footer.png" class="mt32">
>>>>>>> 1c69586bef580f1be2d18659b8b32b1873a7d1d9
		</div>
	</div>
</div>
</template>
<script type="text/javascript">
 
  export default {
    data () {
      return {
        
      }
  }
  }
</script>
